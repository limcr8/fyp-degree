...
def test_anchor_ipfs_hash_uses_web3_transaction() -> None:
    """
    Verifies smart contract anchoring without submitting a real transaction.
    """
    settings = Settings(
    ...
    fake_contract = Mock()
    fake_contract.functions.anchorReport.return_value.build_transaction.return_value = {"to": "0xDEAD"}
    fake_web3.eth.contract.return_value = fake_contract

    with patch("models.integrity_proof._build_web3", return_value=fake_web3):
        proof = anchor_ipfs_hash("report-1", "QmTestCid", "0x" + "34" * 32, settings)
    ...
    fake_web3.eth.contract.assert_called_once()


def test_anchor_ipfs_hash_fallback_without_contract() -> None:
    """
    Verifies fallback memo-anchoring when proof_contract_address is not configured.
    """
    settings = Settings(
        web3_provider_url="https://rpc.example.com",
        web3_private_key="0xabc",
    ...
    assert proof.ipfs_hash == "QmTestCid"
    assert proof.network == "EVM Testnet"
    fake_web3.eth.contract.assert_not_called()
